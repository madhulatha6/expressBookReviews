...
let users = [{ username: "Twinkle", password: "test1234"}];
...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  return res.status(200).json(JSON.stringify(books.books[isbn]));
});
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
    ...
    const username = req.session.authorization.username;
    if (!books.books[isbn].reviews.hasOwnProperty(username)) {
      return res.status(200).json({ message: `Username '${username}' review for ISBN '${isbn}' not found in bookstore.` });
    }
  
    const review = books.books[isbn].reviews[username];
